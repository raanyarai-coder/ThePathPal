...
export const RoadmapAndScorecard: React.FC = () => {
  ...
  return (
    <section id="roadmap" className="py-20 bg-[#0B0F19] text-white border-b border-white/10 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3">
          ...
          <h2 className="text-3xl sm:text-5xl font-black uppercase tracking-tight text-white">
            Built for Scale. <span className="text-[#00F0FF] text-stroke-cyan">Backed by Data.</span>
          </h2>
        ...
        <div className="space-y-6">
          <div className="text-center max-w-xl mx-auto space-y-1">
            <div className="text-xs font-black uppercase tracking-wider text-[#00F0FF]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {PILOT_METRICS.map((item) => (
              <div key={item.category} className="bg-[#121824] p-6 rounded-3xl border border-white/10 space-y-3 hover:border-[#00F0FF]/40 transition-all shadow-xl">
                <div className="flex items-center justify-between">
                  <span className={`text-[10px] font-black px-2.5 py-1 rounded-full uppercase bg-[#00F0FF]/10 text-[#00F0FF] border border-[#00F0FF]/20`}>
                    {item.category}
                  </span>
                  <span className="text-2xl font-black text-[#00F0FF]">{item.metric}</span>
                </div>
        ...
        <div className="bg-[#121824] p-8 rounded-3xl border border-white/10 space-y-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="text-xs font-black uppercase text-[#00F0FF]">SYSTEM SCALE & ROLLOUT</div>
              <h3 className="text-2xl font-black uppercase text-white">6-Phase Enterprise Expansion Plan</h3>
            </div>
            <span className="text-xs font-bold text-gray-400 bg-white/5 px-3 py-1.5 rounded-xl border border-white/10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
            {growthPhases.map((phase) => (
              <div key={phase.num} className="bg-[#0A0D14] p-4 rounded-2xl border border-white/10 space-y-2 hover:border-[#00F0FF]/50 transition-all">
                <div className="flex items-center justify-between">
                  <span className="w-6 h-6 rounded-full bg-[#00F0FF] text-black font-black text-xs flex items-center justify-center">
                    {phase.num}
                  </span>
                  <CheckCircle2 className="w-4 h-4 text-[#00F0FF]" />
                </div>
                <div className="text-xs font-black uppercase text-[#00F0FF]">{phase.title}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {stagedGrowth.map((stage) => (
            <div key={stage.num} className="bg-[#121824] p-5 rounded-2xl border border-white/10 space-y-2">
