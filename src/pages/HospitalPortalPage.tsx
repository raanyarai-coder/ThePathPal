...
export const HospitalPortalPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 animate-fade-in text-white">
      ...
      <div className="bg-[#121824] p-6 sm:p-8 rounded-3xl border-2 border-[#00F0FF]/40 shadow-2xl flex flex-col lg:flex-row items-center justify-between gap-6 overflow-hidden relative">
        <div className="space-y-3 flex-1">
          <div className="flex items-center gap-2">
            <span className="text-xs font-black uppercase tracking-widest text-black bg-[#00F0FF] px-3 py-1 rounded-full flex items-center gap-1.5 shadow-md">
              <Building2 className="w-3.5 h-3.5" />
              HOSPITAL ADMIN HUB
            </span>
            <span className="text-xs font-bold text-[#00F0FF] bg-[#00F0FF]/15 px-3 py-1 rounded-full border border-[#00F0FF]/30 font-mono">
          ...
          <div className="bg-[#1A2232] p-4 rounded-2xl border border-white/10 flex items-center gap-4 max-w-md">
            ...
            <div className="text-right">
              ...
              <span className="text-2xl font-black text-[#00F0FF]">+14.2%</span>
            </div>
          </div>
        </div>

        {/* Generated Hospital Command Center Preview Photo */}
        <div className="w-full lg:w-80 h-48 rounded-2xl overflow-hidden border-2 border-[#00F0FF]/40 shadow-xl relative group shrink-0">
          <img
          ...
          <div className="absolute bottom-2 left-3 right-3 text-xs flex items-center justify-between">
            <span className="text-[10px] font-black uppercase text-[#00F0FF] bg-black/60 px-2 py-0.5 rounded border border-[#00F0FF]/40">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#121824] p-5 rounded-2xl border border-white/10 space-y-1">
          ...
          <div className="text-3xl font-black text-companion-coral">24 Pals Active</div>
        ...
        <div className="bg-[#121824] p-5 rounded-2xl border border-white/10 space-y-1">
          ...
          <div className="text-3xl font-black text-[#00F0FF]">3.2 Mins</div>
        ...
        <div className="bg-[#121824] p-5 rounded-2xl border border-white/10 space-y-1">
          ...
          <div className="text-3xl font-black text-amber-400">98.6%</div>
        ...
        <div className="bg-[#121824] p-5 rounded-2xl border border-white/10 space-y-1">
          ...
          <span className="text-[10px] font-bold text-[#00F0FF]">Tax-Exempt Credit Approved</span>
        </div>
      </div>
      ...
      <div className="bg-[#121824] p-6 sm:p-8 rounded-3xl border border-white/10 space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
          <div>
            <span className="text-xs font-black uppercase text-[#00F0FF] tracking-wider">DISPATCH LOG</span>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="text-xs pl-9 pr-4 py-2.5 rounded-xl bg-[#1A2232] border border-white/15 text-white focus:outline-none focus:ring-2 focus:ring-[#00F0FF]"
              />
            </div>

            <div className="flex items-center gap-1 bg-[#1A2232] p-1 rounded-xl border border-white/15 text-xs font-bold">
              <button
                onClick={() => setFilterStatus('all')}
                className={`px-3 py-1.5 rounded-lg transition-all ${
                  filterStatus === 'all' ? 'bg-[#00F0FF] text-black font-black' : 'text-gray-300 hover:text-white'
                }`}
              >
                All
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-all ${
                  filterStatus === 'matched' ? 'bg-companion-coral text-white font-black' : 'text-gray-300 hover:text-white'
                }`}
              >
                Matched
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-all ${
                  filterStatus === 'in_progress' ? 'bg-navigation-teal text-white font-black' : 'text-gray-300 hover:text-white'
                }`}
              >
                Active
              </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-[#1A2232] border-b border-white/10 text-gray-300 uppercase font-black text-[10px]">
            ...
            <tbody className="divide-y divide-white/5">
              {filteredRequests.map((req) => (
                <tr key={req.id} className="hover:bg-white/5 transition-colors">
                  <td className="p-3 font-mono font-bold text-[#00F0FF]">{req.id}</td>
                  ...
                  <td className="p-3">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${
                      req.status === 'matched'
                        ? 'bg-companion-coral/20 text-companion-coral border border-companion-coral/30'
                        : req.status === 'in_progress'
                        ? 'bg-navigation-teal/20 text-navigation-teal border border-navigation-teal/30'
